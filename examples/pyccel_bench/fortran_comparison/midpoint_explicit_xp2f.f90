! transpiled by xp2f.py from run_midpoint_explicit.py on 2026-09-11 08:29:44
module run_midpoint_explicit_proc_mod
   use python_mod, only: linspace
   use, intrinsic :: iso_fortran_env, only: real64
   implicit none
   private
   integer, parameter :: dp = real64
   public :: dp, humps_deriv, humps_fun, midpoint_explicit, &
...
subroutine midpoint_explicit(dydt, tspan, y0, n, t, y)
   ...
   interface
      pure subroutine midpoint_explicit_dydt_cb_if(cb_a1, cb_a2, cb_a3)
         import dp
         real(kind=dp), intent(in) :: cb_a1
         real(kind=dp), intent(in) :: cb_a2(:)
         real(kind=dp), intent(inout) :: cb_a3(:)
      end subroutine midpoint_explicit_dydt_cb_if
   end interface
   procedure(midpoint_explicit_dydt_cb_if) :: dydt
   real(kind=dp), intent(in) :: tspan(:), y0(:)
   integer, intent(in) :: n
   real(kind=dp), intent(inout) :: t(:), y(:,:)
   real(kind=dp) :: dt, tm
   integer :: i, m
   real(kind=dp), allocatable :: ym(:)
   
   m = size(y0)
   allocate(ym(m), source=0.0_dp)
   ...
   do i = 1, n
      ...
      tm = t(i) + 0.5_dp * dt
      ym = y(i, :) + (0.5_dp * dt) * ym
      call dydt(tm, ym, y(i + 1, :))
      t(i + 1) = t(i) + dt
      y(i + 1, :) = y(i, :) + (dt * y(i + 1, :))
   end do
end subroutine midpoint_explicit

end module run_midpoint_explicit_proc_mod

program run_midpoint_explicit
   use run_midpoint_explicit_proc_mod, only: dp, midpoint_explicit_humps_test
   implicit none
   real(kind=dp) :: err
   
   err = midpoint_explicit_humps_test(0.0_dp, 2000.0_dp, 2000)
   write(*,"(es0.10)") err
end program run_midpoint_explicit
