! transpiled by xp2f.py from run_midpoint_fixed.py on 2026-09-11 08:32:58
module run_midpoint_fixed_proc_mod
   use python_mod, only: linspace
   use, intrinsic :: iso_fortran_env, only: real64
   implicit none
   private
   integer, parameter :: dp = real64
   public :: dp, humps_deriv, humps_fun, midpoint_fixed, &
      & midpoint_fixed_humps_test
contains

function midpoint_fixed_humps_test(t0, t1, n) result(err)
   !
   ...
   call midpoint_fixed(humps_deriv, tspan, y0, n, t, yh)
   err = yh(size(yh,1), 1) - humps_fun(t1)
end function midpoint_fixed_humps_test
...
subroutine midpoint_fixed(dydt, tspan, y0, n, t, y)
   ...
   interface
      pure subroutine midpoint_fixed_dydt_cb_if(cb_a1, cb_a2, cb_a3)
         import dp
         real(kind=dp), intent(in) :: cb_a1
         real(kind=dp), intent(in) :: cb_a2(:)
         real(kind=dp), intent(inout) :: cb_a3(:)
      end subroutine midpoint_fixed_dydt_cb_if
   end interface
   procedure(midpoint_fixed_dydt_cb_if) :: dydt
   ...
   real(kind=dp), allocatable :: y1m(:), y2m(:)
   
   m = size(y0)
   allocate(y1m(m), source=0.0_dp)
   allocate(y2m(m), source=0.0_dp)
   dt = (tspan(2) - tspan(1)) / real(n, kind=dp)
   t(1) = tspan(1)
   y(1, :) = y0
   do i = 1, n
      xm = t(i) + theta * dt
      ...
      t(i + 1) = t(i) + dt
      y(i + 1, :) = (1.0_dp / theta) * y1m + ((1.0_dp - (1.0_dp / theta)) * &
         & y(i, :))
   end do
end subroutine midpoint_fixed

end module run_midpoint_fixed_proc_mod

program run_midpoint_fixed
   use run_midpoint_fixed_proc_mod, only: dp, midpoint_fixed_humps_test
   implicit none
   real(kind=dp) :: err
   
   err = midpoint_fixed_humps_test(0.0_dp, 2000.0_dp, 2000)
   write(*,"(es0.10)") err
end program run_midpoint_fixed
