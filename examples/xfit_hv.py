...
the script reads a csv of prices, computes log returns, computes 20-day
historical volatility using either equal or linearly declining weights,
then fits the linear predictor a + b*hv for future realized volatility.
all calculations and file handling are done without pandas.
"""
...
def read_price_csv(path):
    """read a csv whose first column is dates and remaining columns are prices."""
    ...
    asset_names = [name.strip() for name in header[1:]]
    data_rows = rows[1:]
    ...
    for i, row in enumerate(data_rows, start=2):
        if len(row) != n_assets + 1:
            raise ValueError(
                f"row {i} has {len(row)} fields but expected {n_assets + 1}"
            )
...
def analyze_file(path, lookback, horizons, weights, annualization):
    ...
    _, asset_names, prices = read_price_csv(path)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "read a csv of prices, compute hv from log returns, and fit "
            "future_vol = a + b*hv for each asset and horizon"
        )
    )
    parser.add_argument(
        "csv_file",
        nargs="?",
        default="spy_efa_eem_tlt_lqd.csv",
        help="csv file of prices; first column must be dates/index",
    )
    parser.add_argument(
        "--lookback",
        type=int,
