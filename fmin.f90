! Vendored from https://github.com/jacobwilliams/fmin
! (Jacob Williams, BSD-3-Clause license). A modern-Fortran port of the
! classic netlib fmin.f (Forsythe/Malcolm/Moler, derived from Richard
! Brent's golden-section + parabolic-interpolation algorithm) -- the same
! algorithm scipy.optimize.minimize_scalar(method='bounded') itself
! implements. The REAL32/64/128 preprocessor conditionals are stripped down
! to a fixed real64 since xp2f.py's helper-file build doesn't pass -cpp;
! otherwise the module is unmodified.
!*****************************************************************************************
!>
...
!      changed using optional preprocessor flags in the upstream project.
!      This vendored copy drops the preprocessor conditionals (xp2f.py's
!      helper-file build doesn't pass -cpp) and is fixed at real kind:
