...
# is project-maintenance tooling -- it shells out to run the other scripts
# and pytest via `subprocess`, checks exit codes and file layout via
# `pathlib`/`shutil`. There is no numeric computation to translate; a
# Fortran transpiler has nothing to do with running a smoke-test harness.
...
PROJECT_FILES = [
    ...
    "xcompare_vendors.py",
    "xconsolidate.py",
    "xresample_intraday.py",
    "xsplice_prices.py",
    "xread_prices.py",
    "xfetch_yahoo_intraday.py",
    "xfetch_databento_intraday.py",
    "xrealized_vol_by_frequency.py",
    "xtime_boundary_effects.py",
]
