#!/usr/bin/env python3
# TRANSLATION NOTES: not converted into an xp2f.py translation target. This
# script operates on zip archives and raw CSV rows as list[dict[str, str]]
# (via `zipfile`/`csv`/`re`) for directory scanning, symbol lookup, and
# row-level dedup -- it has no pandas/numpy arrays and no numeric kernel;
# it is entirely file-format and filesystem-layout wrangling, which is
# outside a numeric Fortran transpiler's scope regardless of simplification.
# Left as an unmodified copy of the original.
"""
Consolidate intraday price snapshots from multiple vendors.
...
Rows are processed in ascending snapshot-date order. If the same bar appears
in multiple snapshots, the row from the newest selected snapshot is kept.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Consolidate selected symbols from dated intraday price snapshots."
    )
    parser.add_argument(
        "symbols",
        nargs="*",
...
def run_yahoo(args: argparse.Namespace, output_dir: Path) -> None:
    ...
    write_outputs(args, output_dir, symbol_files, snapshots, consolidate_yahoo_symbol)
