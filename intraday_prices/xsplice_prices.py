#!/usr/bin/env python3
# TRANSLATION NOTES: not converted into an xp2f.py translation target. Like
# xconsolidate.py, this script operates on zip archives and raw CSV rows as
# list[dict[str, str]] (via `zipfile`/`csv`/`re`) for directory scanning and
# row-level dedup/rebasing -- no pandas/numpy arrays, no numeric kernel
# suitable for a Fortran transpiler. (estimate_overlap_ratio/median_value
# are the only genuinely numeric pieces, and they're too small and too
# entangled with the dict-row plumbing around them to extract on their own.)
# Left as an unmodified copy of the original.
"""
...
def select_available_symbols(
    requested_symbol_files: list[str],
    available_symbol_files: list[str],
) -> tuple[list[str], list[str]]:
    """Return requested symbols found in snapshots plus requested symbols missing everywhere."""
    available_by_key = {symbol.lower(): symbol for symbol in available_symbol_files}
    selected: list[str] = []
    missing: list[str] = []
    for symbol_file in requested_symbol_files:
        available = available_by_key.get(symbol_file.lower())
        if available is None:
            missing.append(symbol_file)
        else:
...
def run_stooq(args: argparse.Namespace, output_dir: Path) -> None:
    ...
    if args.all:
        symbol_files = list_stooq_symbols(snapshots)
    else:
        requested_symbol_files = [normalize_stooq_symbol(symbol) for symbol in args.symbols]
        requested_symbol_files = apply_limit(requested_symbol_files, args.limit)
        requested_count = len(requested_symbol_files)
        symbol_files, missing_symbol_files = select_available_symbols(
            requested_symbol_files,
            list_stooq_symbols(snapshots),
        )
        if not symbol_files:
            raise SystemExit("None of the requested symbols were found in any selected Stooq archive.")
...
def run_yahoo(args: argparse.Namespace, output_dir: Path) -> None:
    ...
    missing_symbol_files: list[str] = []
    if args.all:
        symbol_files = list_yahoo_symbols(snapshots, args.input_glob)
    else:
        requested_symbol_files = [
            normalize_yahoo_symbol(symbol, args.symbol_file_template) for symbol in args.symbols
        ]
        requested_symbol_files = apply_limit(requested_symbol_files, args.limit)
        requested_count = len(requested_symbol_files)
        symbol_files, missing_symbol_files = select_available_symbols(
        ...
        if not symbol_files:
            raise SystemExit("None of the requested symbols were found in any selected Yahoo directory.")
    symbol_files = apply_limit(symbol_files, args.limit)
...
def write_outputs(
    ...
    consolidate_func,
    requested_count: int | None = None,
    missing_symbol_files: list[str] | None = None,
) -> None:
    if requested_count is None:
        print(f"Symbols selected: {len(symbol_files)}")
    else:
        print(f"Symbols selected: {len(symbol_files)} out of {requested_count}")
    if missing_symbol_files:
        print(f"Symbols missing from all selected snapshots: {', '.join(missing_symbol_files[:10])}")
    if symbol_files:
