! Vendored from https://github.com/jacobwilliams/lbfgsb
! (Jacob Williams, BSD-3-Clause license -- a modernized refactoring of the
! original Nocedal/Byrd/Lu/Zhu L-BFGS-B, itself BSD/public-domain; see
! License.txt in the upstream repo). This is the exact algorithm
! scipy.optimize.minimize(method='L-BFGS-B') wraps via a compiled Fortran
! extension.
!
! Bundled from the upstream src/ four-file layout (lbfgsb_kinds_module,
! lbfgsb_blas_module, lbfgsb_linpack_module, lbfgsb_module) into this single
! file so xp2f.py's auto-helper-discovery (which maps `use lbfgsb_module`
! to a single lbfgsb.f90) can find every dependency without needing to
! resolve transitive `use` statements across helper files -- same reason
! dataframe_index_date.f90 bundles its own four modules.
!
! Two edits from upstream: the REAL32/64/128 preprocessor conditional in
! lbfgsb_kinds_module is stripped down to a fixed real64, and the
! #ifndef HAS_BLAS / #endif guard in lbfgsb_blas_module is removed (kept
! the guarded body, which is what compiles when HAS_BLAS is undefined --
! xp2f.py's helper-file build doesn't pass -cpp). Otherwise unmodified.
!
! See lbfgsb_bridge.f90 for xp2f.py's reverse-communication driver loop
! around setulb().
