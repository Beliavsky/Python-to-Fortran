...
class InputExpansionError(Exception):
    """Invalid batch input such as a missing @list file."""
...
def _default_tee_prefix(inputs: list[str]) -> str:
    """Return a short prefix from the first input/list filename."""
    if not inputs:
        return "results"
    first = str(inputs[0])
    if first.startswith("@"):
    ...
    if len(words) >= 2:
        return "_".join(words[:2]) + "_results"
    if words:
        return words[0] + "_results"
    return "results"
...
def _parse_stage_times(text: str) -> dict[str, float]:
    ...
    for ln in text.splitlines():
        if ln.strip().lower() == "timing summary (seconds):":
            in_timing = True
            saw_table_header = False
            continue
        if not in_timing:
        ...
        if stripped.lower().startswith("stage"):
            ...
            continue
        if saw_table_header and not re.match(r"^(python run|transpile|compile|fortran run|total)\b", stripped, flags=re.IGNORECASE):
            break
...
_MODULE_DEF_RE = re.compile(r"^\s*module\s+(?!procedure\b)([A-Za-z_]\w*)\b", flags=re.IGNORECASE | re.MULTILINE)


def _modules_defined_in_source(src_text: str) -> list[str]:
...
def _print_blocker_report(results: List[CaseResult], top_n: int = 0) -> None:
    ...
    calls_sorted = sorted(call_to_files.items(), key=lambda kv: (-len(kv[1]), kv[0]))
    if top_n > 0:
        calls_sorted = calls_sorted[:top_n]
        print(f"Top {top_n} unsupported calls by unique-file impact:")
    else:
        print("Unsupported calls by unique-file impact:")
    if not calls_sorted:
        print("(none)")
    else:
    ...
    if top_n > 0:
        ...
        print("Greedy implementation order:")
    covered = 0
    step = 0
    while unresolved and items and (top_n <= 0 or step < top_n):
        best_key = None
        best_gain = -1
    ...
    if top_n > 0:
        ...
        print(f"Remaining unresolved files after greedy order: {len(unresolved)}")


def _read_input_list(list_path: Path) -> List[str]:
    """Read one @list file, ignoring blank lines and comments."""
    if not list_path.exists():
        raise InputExpansionError(f"@list file not found: {list_path}")
    if not list_path.is_file():
        raise InputExpansionError(f"@list path is not a file: {list_path}")
    try:
        text = list_path.read_text(encoding="utf-8", errors="ignore")
    except OSError as exc:
        raise InputExpansionError(f"could not read @list file {list_path}: {exc}") from exc
    out: List[str] = []
    for ln in text.splitlines():
        raw = ln.strip()
...
def main() -> int:
    ...
    ap.add_argument(
        "--compiler",
        default="gfortran -O3 -march=native -Wfatal-errors",
    ...
    ap.add_argument(
        "--time-summary",
        action="store_true",
        help="Request xp2f stage timings and print per-file python/transpile/compile/Fortran-run times.",
    )
    ...
    ap.add_argument(
        ...
        nargs="?",
        const="",
        help="Also write xp2f_batch console output to this file; omit file to use <input-prefix>_results_YYYYMMDD_HHMMam.txt.",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="Limit blocker lists to top N items (0 = no limit; 20 gives prior behavior).",
    )
    args = ap.parse_args()
    ...
    if args.time_both and args.no_run:
        ...
        return 1
    if args.time_summary and args.no_run:
        print("Invalid options: --time-summary cannot be combined with --no-run.")
        return 1
    ...
    raw_helper_args = list(args.helpers)
    if not raw_helper_args:
        repo_dir = Path(__file__).resolve().parent
        for helper_name in ("python.f90", "lapack_d.f90"):
            helper_path = repo_dir / helper_name
            if helper_path.exists():
                raw_helper_args.append(str(helper_path))
    helper_args = [
        str((cwd_resolved / h).resolve()) if not Path(h).is_absolute() else str(Path(h).resolve())
        for h in raw_helper_args
    ]
    ...
    def _run_case(i: int, pyf: Path) -> CaseResult:
        ...
        if (not args.strict) and (not args.strict_fix):
            want_timing = args.time_both or args.timing_breakdown or args.time_summary
            if want_timing:
                if args.time_both or args.run_both or args.run_diff:
        ...
        quiet_python_time = None
        if (
            args.time_summary
            and not args.strict
            and not args.strict_fix
            ...
            try:
                ...
                pass
            quiet_python_time = time.perf_counter() - t_py
        ...
        if quiet_python_time is not None:
            stage_times["python run"] = quiet_python_time
            if "total" in stage_times:
                stage_times["total"] += quiet_python_time
        out_lines: list[str] = []
        if ok:
            if args.strict:
                status = "STRICT_PASS"
        ...
        return CaseResult(
            ...
            output="\n".join(out_lines),
            strict_file_created=bool(args.strict_fix and ok and status == "STRICT_FIXED"),
        )

    def _format_case_times(r: CaseResult) -> str:
        def fmt(stage: str) -> str:
            if not r.stage_times or stage not in r.stage_times:
                return "       -"
            return f"{r.stage_times[stage]:8.3f}"
        ...
        if r.stage_times:
            ...
            ft_t = r.stage_times.get("fortran run")
            if py_t is not None and ft_t is not None and py_t != 0.0:
                ratio = ft_t / py_t
    ...
    if args.jobs == 1:
        ...
        print(f"Jobs: running up to {args.jobs} xp2f.py subprocesses concurrently.")
        pending = {}
        next_idx = 1
        with ThreadPoolExecutor(max_workers=args.jobs) as executor:
            ...
            while pending:
                ...
                if not done:
                    active = ", ".join(
                        f"[{idx}/{total}] {py_files[idx - 1]}"
                        for idx in sorted(pending.values())
                    )
